...
ostream& operator<<(ostream& os, DiscardPile& discardPile){
    if (discardPile.pile.empty()) //is pile empty?
        return os;
    auto card = discardPile.pile.back(); //get the last card
    if (card != nullptr) //is the card a nullptr?
        os << *card; //print the card to the ostream
...
DiscardPile& DiscardPile::operator+=(Card* card){ // add a card to the discard pile, this function returns a reference to the discard pile
    if (card != nullptr)
        pile.push_back(card);
    return *this;
}
